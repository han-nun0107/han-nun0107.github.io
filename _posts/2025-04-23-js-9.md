...
```js
const original = { name: "철수", age: 15 };
const copy = Object.assign({}, original);
copy.age = 20;
console.log(original.age); // 15
```
...
`객체나 배열을 "펼쳐서" 복사허가나 병합할 때 사용`하는 문법(es6부터 도입)
...
const deepCopy = JSON.parse(JSON.stringify(original)); // JSON.stringify(original) -> orginal 객체를 JSON 문자열로 변환 -> JSON.parse() -> JSON 문자열을 다시 객체로 변환, 새롭게 만들어진 객체라 원본과 완전히 독립
...
```js
function greet() {
  console.log("Hello"); // 정상 출력 하려면 return "Hello"
}
const result = greet();
